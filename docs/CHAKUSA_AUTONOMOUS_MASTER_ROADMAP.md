...
**Last roadmap position:** #18–#21 complete. OA-6 resolved (`docs/OWNER_DECISION_OA6_STAGE22_WEB.md`) — #22 built on `website/` Astro + `cloudflare/auth-gateway`. #22 V1 delivered: 9/13 high-value workflows usable end-to-end from the web (dashboard, calendar ops, leads, customers, inbox+reply, inventory, reviews+reply, AI receptionist, settings), 4 view-only with named deferrals (quote/invoice creation, team admin, report detail); capability-aware UI + gateway allowlist expansion + drift tests. #23 Production Hardening complete — all 14 areas audited; P1 worker defects fixed (HTTP-tick job parity, step isolation + fail-safe, stranded-dispatch recovery); correlation ids added; `docs/PRODUCTION_HARDENING.md` audit + runbook. No P0/P1 open. Production migration backlog is 10, owner-blocked (OA-1); app-link identifiers owner-blocked (OA-5, non-blocking); DB pool/timeout params to apply at deploy (OA-7, non-blocking). #24 Security & Privacy Audit complete (P1 deps fixed, no other P0/P1). #25 Manual QA Preparation complete — `docs/RELEASE_QA_MATRIX.md` (17-section device matrix) + `scripts/seed-qa.ts` (guarded no-secrets seed). #26 Release Readiness engineering complete to the owner boundary — `docs/RELEASE_READINESS_CHECKLIST.md`, wrong-domain link P1 fixed, `expo install --check` clean; no engineering P0/P1. Remaining gates (migration deploy, Android/iOS release builds, store declarations, physical-device QA) are owner actions — OA-8 (+ OA-1/2/3/5/7). **V1 engineering is release-ready.** Post-V1 backlog (§9) not started.
