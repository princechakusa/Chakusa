...
export function ActivationJourneyCard({ journey, onContinue }: { journey: ActivationJourney; onContinue: () => void }) {
  ...
  return <View style={styles.card} accessibilityLabel={`First value journey, ${journey.complete} of ${journey.total} complete`}>
    <View style={styles.header}><View style={styles.headerCopy}><Text style={styles.eyebrow}>YOUR FIRST CHAKUSA WIN</Text><Text style={styles.title}>Turn a booking into a business result</Text></View><StatusBadge label={`${journey.complete}/${journey.total}`} /></View>
    ...
    <Text style={styles.hint}>These steps update only from real activity. Finish the journey to see the booking, payment, communication, and reputation value Chakusa delivered.</Text>
  </View>;
}
