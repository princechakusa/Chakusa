...
export function CountryPhoneInput({ value, onChange, label = 'Phone', defaultCountryCode = 'ZW' }: { value: string; onChange: (value: string) => void; label?: string; defaultCountryCode?: string }) {
  ...
  const choose = (next: Country) => { setCountry(next); onChange(normalizePhone(next.dial, local)); setOpen(false); setSearch(''); };
