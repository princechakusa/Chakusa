...
describe('public website routes', () => {
  ...
  it('preserves secure public quote tokens and malformed behavior', () => { expect(publicRouteFromPath('/q/id.secret')).toEqual({ kind: 'quote', token: 'id.secret' }); expect(publicRouteFromPath('/q')).toEqual({ kind: 'quote', token: null }); expect(publicRouteFromPath('/q/bad/extra')).toEqual({ kind: 'quote', token: null }); });
  it('preserves secure public invoice tokens and malformed behavior', () => { expect(publicRouteFromPath('/i/id.secret')).toEqual({ kind: 'invoice', token: 'id.secret' }); expect(publicRouteFromPath('/i')).toEqual({ kind: 'invoice', token: null }); expect(publicRouteFromPath('/i/bad/extra')).toEqual({ kind: 'invoice', token: null }); });
  ...
  it('preserves secure public booking-management tokens', () => { expect(publicRouteFromPath('/b/janes-plumbing/booking/token.secret')).toEqual({ kind: 'business-booking', slug: 'janes-plumbing', token: 'token.secret' }); });
