...
export function BusinessSettingsScreen() {
  ...
  const [messagingConsent, setMessagingConsent] = useState(Boolean(business?.messagingConsentConfirmedAt)); const [paymentReminders, setPaymentReminders] = useState(Boolean(business?.paymentRemindersEnabled)); const [stripeStatus, setStripeStatus] = useState<{ connected: boolean; chargesEnabled: boolean; payoutsEnabled: boolean } | null>(null); const [connectingStripe, setConnectingStripe] = useState(false);
