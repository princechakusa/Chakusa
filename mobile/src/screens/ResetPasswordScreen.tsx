...
export function ResetPasswordScreen() {
  ...
  const submit = async () => { if (submissionActive.current) return; if (!token) { setError('This reset link is invalid. Request a new one.'); return; } if (password !== confirm) { setError('Passwords do not match.'); return; } submissionActive.current = true; setLoading(true); setError(null); try { await resetPassword(token, password); } catch (caught) { setError(caught instanceof ApiError ? caught.message : 'Unable to reset your password.'); } finally { submissionActive.current = false; setLoading(false); } };
