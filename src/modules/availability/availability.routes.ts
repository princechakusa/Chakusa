...
export default async function availabilityRoutes(fastify: FastifyInstance) {
  ...
  fastify.post("/blocks", async (request, reply) => { requireCapability(request, "availability.manage"); reply.status(201).send(await createBookingBlock(request.businessId!, request.user.userId, createBookingBlockSchema.parse(request.body))); });
  fastify.delete("/blocks/:id", async (request, reply) => { requireCapability(request, "availability.manage"); await deleteBookingBlock(request.businessId!, idParams.parse(request.params).id); reply.status(204).send(); });
  fastify.patch("/members/:id/hours", async (request, reply) => { requireCapability(request, "availability.manage"); const input = memberHoursSchema.parse(request.body); reply.send(await updateMemberWorkingHours(request.businessId!, idParams.parse(request.params).id, input.workingHours)); });
}
