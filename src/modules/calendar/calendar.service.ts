...
export async function resolveCalendarFeed(rawToken: string, now = new Date()) {
  ...
  const subscription = await prisma.externalCalendarSubscription.findUnique({
    where: { tokenId },
    select: { id: true, tokenHash: true, businessId: true, revokedAt: true, business: { select: { name: true, platformStatus: true } } },
  });
  ...
  return ["BEGIN:VCALENDAR", "VERSION:2.0", "PRODID:-//Chakusa//Business Calendar//EN", "CALSCALE:GREGORIAN", "METHOD:PUBLISH", ...events, "END:VCALENDAR", ""].join("\r\n");
}
