...
import { createAttachmentDownload, downloadAttachment, initiateAttachment, uploadAttachment } from "../../lib/messaging/attachmentPlatform.js";
...
export default async function messageRoutes(fastify: FastifyInstance) {
  ...
  fastify.post("/credentials", async (request, reply) => { requireBusinessRole(request, ["OWNER"]); const input = z.object({ channelAccountId: z.string().uuid(), credential: z.record(z.string(), z.unknown()).refine(value => Object.keys(value).length > 0) }).parse(request.body); const verifier = { verify: async (_provider: string, value: Record<string, unknown>) => ({ valid: Object.values(value).every(item => typeof item === "string" && item.length > 0), latencyMs: 0 }) }; const created = await storeProviderCredential(request.businessId!, request.user.userId, input.channelAccountId, input.credential, verifier); reply.status(201).send({ id: created.id, keyVersion: created.keyVersion, validationStatus: created.validationStatus, createdAt: created.createdAt }); });
