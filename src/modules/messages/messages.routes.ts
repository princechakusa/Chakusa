...
export default async function messageRoutes(fastify: FastifyInstance) {
  ...
  fastify.post("/attachments/mobile", { config: { bodyLimit: 28 * 1024 * 1024 } }, async (request, reply) => { const input = z.object({ fileName: z.string().min(1).max(255), mimeType: z.string().min(3).max(120), dataBase64: z.string().min(1).max(28 * 1024 * 1024) }).parse(request.body); const body = Buffer.from(input.dataBase64, "base64"); if (!body.length || body.length > 20 * 1024 * 1024) throw ApiError.badRequest("Attachment exceeds the 20 MB limit"); const declared = { fileName: input.fileName, mimeType: input.mimeType, sizeBytes: body.length, checksumSha256: createHash("sha256").update(body).digest("hex") }; const initiated = await initiateAttachment(request.businessId!, declared); const attachment = await uploadAttachment(initiated.uploadToken, body, input.mimeType); reply.status(202).send({ id: attachment.id, uploadStatus: attachment.uploadStatus, malwareScanStatus: attachment.malwareScanStatus }); });
  ...
  fastify.post("/attachments/:id/rescan", async (request, reply) => { requireCapability(request, "messaging.config.manage"); const { id } = z.object({ id: z.string().uuid() }).parse(request.params); await rescanAttachment(request.businessId!, id); reply.status(202).send({ queued: true }); });
  ...
  fastify.post("/templates", async (request, reply) => { requireCapability(request, "messaging.config.manage"); const input = z.object({ key: z.string().regex(/^[A-Z0-9_]{2,80}$/), name: z.string().min(1).max(120), purpose: z.string().min(1).max(40), industry: z.string().max(80).optional(), channel: z.enum(["sms", "whatsapp"]), locale: z.string().regex(/^[a-z]{2}(?:-[A-Z]{2})?$/), body: z.string().min(1).max(5000), subject: z.string().max(200).optional(), variables: z.array(z.string().regex(/^[a-zA-Z][\w.]*$/)).max(50) }).parse(request.body); reply.status(201).send(await createMessagingTemplate(request.businessId!, input)); });
  fastify.post("/templates/:id/versions", async (request, reply) => { requireCapability(request, "messaging.config.manage"); const { id } = z.object({ id: z.string().uuid() }).parse(request.params); const input = z.object({ channel: z.enum(["sms", "whatsapp"]), locale: z.string(), body: z.string().min(1).max(5000), subject: z.string().max(200).optional(), variables: z.array(z.string()).max(50) }).parse(request.body); reply.status(201).send(await createTemplateVersion(request.businessId!, id, input)); });
  fastify.post("/templates/versions/:id/publish", async (request) => { requireCapability(request, "messaging.config.manage"); return publishTemplate(request.businessId!, z.object({ id: z.string().uuid() }).parse(request.params).id); });
  fastify.post("/templates/:id/rollback", async (request) => { requireCapability(request, "messaging.config.manage"); const { id } = z.object({ id: z.string().uuid() }).parse(request.params); const input = z.object({ version: z.number().int().positive(), channel: z.string(), locale: z.string() }).parse(request.body); return rollbackTemplate(request.businessId!, id, input.version, input.channel, input.locale); });
  ...
  fastify.get("/credentials", async (request) => { requireCapability(request, "messaging.config.manage"); return maskedCredentials(request.businessId!); });
  fastify.post("/credentials", async (request, reply) => { requireCapability(request, "messaging.credentials.manage"); const input = z.object({ channelAccountId: z.string().uuid(), credential: z.record(z.string(), z.unknown()).refine(value => Object.keys(value).length > 0) }).parse(request.body); const account = await prisma.messagingChannelAccount.findFirst({ where: { id: input.channelAccountId, businessId: request.businessId!, deletedAt: null }, select: { provider: true } }); const verifier = account ? getProviderCredentialVerifier(account.provider) : undefined; if (!verifier) throw ApiError.serviceUnavailable("Provider credential verification is not configured"); const created = await storeProviderCredential(request.businessId!, request.user.userId, input.channelAccountId, input.credential, verifier); reply.status(201).send({ id: created.id, keyVersion: created.keyVersion, validationStatus: created.validationStatus, createdAt: created.createdAt }); });
  ...
  fastify.post("/conversations/bulk", async (request) => { requireCapability(request, "messaging.config.manage"); const input = z.object({ ids: z.array(z.string().uuid()).min(1).max(100), status: z.string().optional(), assignedMemberId: z.string().uuid().nullable().optional(), archive: z.boolean().optional() }).parse(request.body); return { updated: await bulkConversationUpdate(request.businessId!, input.ids, input, request.user.userId) }; });
  fastify.post("/conversations/:id/merge", async (request) => { requireCapability(request, "messaging.config.manage"); const { id } = z.object({ id: z.string().uuid() }).parse(request.params); const { sourceIds } = z.object({ sourceIds: z.array(z.string().uuid()).min(1).max(20) }).parse(request.body); return mergeConversations(request.businessId!, id, sourceIds, request.user.userId); });
  fastify.post("/conversations/:id/split", async (request, reply) => { requireCapability(request, "messaging.config.manage"); const { id } = z.object({ id: z.string().uuid() }).parse(request.params); const { messageIds } = z.object({ messageIds: z.array(z.string().uuid()).min(1).max(100) }).parse(request.body); reply.status(201).send(await splitConversation(request.businessId!, id, messageIds, request.user.userId)); });
  fastify.post("/conversations/:id/slas", async (request, reply) => { requireCapability(request, "messaging.config.manage"); const { id } = z.object({ id: z.string().uuid() }).parse(request.params); const input = z.object({ firstResponse: z.number().int().positive(), resolution: z.number().int().positive(), assignment: z.number().int().positive(), escalation: z.number().int().positive() }).parse(request.body); reply.status(201).send(await initializeConversationSLAs(request.businessId!, id, input)); });
