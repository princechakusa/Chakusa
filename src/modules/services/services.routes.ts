...
export default async function serviceOfferingRoutes(fastify: FastifyInstance) {
  ...
  fastify.post("/", async (request, reply) => { requireCapability(request, "catalog.manage"); reply.status(201).send(await createServiceOffering(request.businessId!, createServiceOfferingSchema.parse(request.body))); });
  fastify.patch("/:id", async (request, reply) => { requireCapability(request, "catalog.manage"); reply.send(await updateServiceOffering(request.businessId!, idParams.parse(request.params).id, updateServiceOfferingSchema.parse(request.body))); });
  fastify.delete("/:id", async (request, reply) => { requireCapability(request, "catalog.manage"); reply.send(await archiveServiceOffering(request.businessId!, idParams.parse(request.params).id)); });
}
